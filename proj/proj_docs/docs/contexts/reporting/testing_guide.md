...
Brief: Complete testing strategy for Reporting context. Six parts plus fixtures covering report generation, Present/Late classification, CSV/Excel export, file downloads, authorization (lecturer vs admin), and cross-context integration. Focus on business logic correctness and access control.
...
- **Classification logic**: Present/Late must be correct
...
**Why Important**: Accurate Late student count (any non-present)
...
**Setup**:
 ...
 - 35 have attendance records with status="present"
 - 15 have status="late" or no attendance record

**Assertions**:
- `total_students` = 50
- `present_count` = 35, `present_percentage` = 70.0
- `late_count` = 15, `late_percentage` = 30.0

**Why Important**: Report header summary (two-status model Present/Late)
...
### test_classify_students_some_late

**Purpose**: Missing attendance or late attendance classified as Late

**Setup**:
- 10 eligible students
- 7 have attendance with status="present"
- 3 do not have attendance (should be Late)

**Assertions**:
- 7 classified as "Present"
- 3 classified as "Late" (missing attendance)
- Late students without attendance have NULL time_recorded, within_radius, latitude, longitude

**Why Important**: **Core business logic** - Late detection (any non-present)
...
**Expected (updated rule)**: Any status other than "present" results in "Late" (reporting trusts status field from attendance-recording; no re-validation). 
...
**Setup**:
- Attendance with status="late", is_within_radius=False

**Expected**: Returns "Late". The `within_radius` flag should be False and included in the row for diagnostics. Attendance outside the radius is treated as Late (not Present).

**Why Important**: Diagnostics - record location accuracy separately from presence classification; being outside the radius remains Late.

---

### test_classify_single_student_no_attendance

**Purpose**: No attendance record classified as Late
...
**Setup**:
- Report data with students (Present, Late)
 - Report data with students (Present, Late)
...
- Late students without attendance have empty fields for time/location
...
- Header row is bold
 - Status cells color-coded (Present=Green, Late=Orange)
...
**Purpose**: All students Late (no attendance records)
...
- Statistics: late_count=10, present_count=0
...
**Setup**:
- 10 students, all with is_within_radius=False and status="late"

**Assertions (updated rule)**:
- All 10 classified as "Late"
- `present_count` = 0, `late_count` = 10

**Note**: Attendance outside the radius remains Late; diagnostics still capture within_radius.
...
**Setup**:
- Late students without attendance (NULL time, location)
...
**Purpose**: Create varied attendance (present, late) with diagnostic flags

**Returns**: List of Attendance instances

**Breakdown**:
- 5 "present", is_within_radius=True
- 3 "late" (e.g., outside radius or window), is_within_radius=False
- 2 no attendance record (these students will be Late)
**Note**: Only the first group counts as Present in statistics; all others are Late.
---
...
1. **Classification logic** - Present/Late accuracy
...
- Edge cases (no students, all late/no attendance)
...
**Single Test**: `pytest tests/reporting/test_services.py::test_classify_students_some_late`

---

## Key Takeaways

1. **Classification logic testing** - Present/Late correctness critical
...
6. **Edge cases** - No students, all late/no attendance, large datasets
